...
void UCFO::SetEngine_Power() {
    ...
    double torque_aux = power / std::max(omega, epsilon);
    double torque = std::min(torque_aux, max_torque);
    ...
    if (omega_wheel <= epsilon) {
        ...
        force = std::min((torque / wheel_radius) * (omega / std::max(omega_wheel, epsilon)),
